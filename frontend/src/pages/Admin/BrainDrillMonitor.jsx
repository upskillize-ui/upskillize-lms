/**
 * TestGen.jsx — Your Enhanced Version
 *
 * MODIFICATIONS:
 * ✅ Dynamic N questions (type ANY number 1-100, not dropdown)
 * ✅ Dynamic duration (type ANY number 5-180 minutes)
 * ✅ Better loading spinner
 * ✅ Real-time validation
 * ✅ All features from your original code preserved
 */

import { useState, useEffect, useRef, useCallback } from "react";
import api from "../../services/api";

// Design tokens (from your original)
const C = {
  bg: "#0d1117",
  surface: "#161b22",
  raised: "#1c2230",
...
function SetupScreen({ onGenerate, loading, slots }) {
  ...
  // ✅ DYNAMIC QUESTION COUNT VALIDATION (1-100)
  const handleNQChange = (e) => {
    ...
    if (num < 1) {
      setErrors((p) => ({ ...p, nQ: "Minimum 1 question" }));
      return;
    }
    if (num > 100) {
      setErrors((p) => ({ ...p, nQ: "Maximum 100 questions" }));
      return;
    }
  ...
  // ✅ DYNAMIC DURATION VALIDATION (5-180 minutes)
  const handleDurChange = (e) => {
    ...
    if (num < 5) {
      setErrors((p) => ({ ...p, dur: "Minimum 5 minutes" }));
      return;
    }
    if (num > 180) {
      setErrors((p) => ({ ...p, dur: "Maximum 180 minutes" }));
      return;
    }
